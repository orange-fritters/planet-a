...
class SiamUnet_conc_multi(nn.Module):
    ...
    def __init__(self, input_nbr, label_nbr):
        ...
        ############################### Channel Pick #################################
        self.channel_picker = nn.Parameter(torch.ones((1, self.input_nbr_1, 1, 1)))
        self.channel_bottle = nn.Sequential(
            nn.Conv2d(in_channels=self.input_nbr_1,
                      out_channels=self.input_nbr_1 // 3,
                      kernel_size=1,
                      padding=0),
            nn.ReLU(),
            nn.Conv2d(in_channels=self.input_nbr_1 // 3,
                      out_channels=self.input_nbr_1 // 3,
                      kernel_size=3,
                      padding=1),
            nn.ReLU(),
            nn.Conv2d(in_channels=self.input_nbr_1 // 3,
                      out_channels=self.input_nbr_1,
                      kernel_size=1,
                      padding=0))
    ...
    def forward(self, s2_1, s2_2, s1_1, s1_2):
        ...
        #################################################### encoder S2 ####################################################
        # Stage 0 channel picker
        s2_1 = self.channel_picker*s2_1
        s2_1 = self.channel_bottle(s2_1)
        ...
        ####################################################
        # Stage 0 channel pick
        s2_2 = self.channel_picker * s2_2
        s2_2 = self.channel_bottle(s2_2)
