...
class RemoteSenseData(Dataset):
    ...
    Args:
        root (Path)       : data path,
        transform (Any)   : Whether to transform the image,
        val=False (Bool)  : Whether the loader is used in validation phase,
        fp_modifier (int) : Weight modifier implemented on Loss
    """
    def __init__(self, root, transform=None, val=False, fp_modifier=2):
        # Sample List
        self.file_names = []
        self.true_pix = 0
        self.n_pix = 0
